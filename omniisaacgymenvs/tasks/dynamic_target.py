...
class JetbotTask(RLTask):
    def __init__(
        ...
        self.target_pose = Gf.Vec3f(3.3, 0, 0.0)
        self.target_size = Gf.Vec3f(.05, .05, .01)
        self.target_color = Gf.Vec3f(1, 0, 0)
        self.target_ori = Gf.Quatf(0.95, 0, 0, -0.31)
    ...
    def set_up_scene(self, scene) -> None:
        ...
        # self._targets.set_collision_enabled(False)
    ...
    def add_target(self):
        cubePrim = omni.physx.scripts.physicsUtils.add_cube(self._stage, self.default_zero_env_path + "/Target/target", self.target_size, self.target_pose,  color = self.target_color)
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        object_paths = self._targets.prim_paths


        for i in range(self._num_envs):
            controls[i] = self._diff_controller.forward([actions[i][0].item()+0.2, actions[i][1].item()])
            self.set_obstacle_properties(self._stage, object_paths[i], -0.35, 0, 0)  
    ...
    def reset_idx(self, env_ids):
        ...
        #randomize initial jet_pose in range of [-1,1]
        self.rand_pose = torch.zeros(3)
        rand_pos = ((torch.rand(3) * 2) - 1)
        self.rand_pose[0],self.rand_pose[1],self.rand_pose[2] = rand_pos[0], rand_pos[1], 0
